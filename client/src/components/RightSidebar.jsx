      ...
      <div className="bg-[#8185B2]/10 text-white w-full relative overflow-y-scroll max-md:hidden">
        <div className="pt-16 flex flex-col items-center gap-2 text-xs font-light mx-auto">
          ...
          <h1 className="px-10 text-xl font-medium mx-auto flex items-center gap-2">
            <span className={`w-2 h-2 rounded-full ${isOnline ? "bg-green-500" : "bg-neutral-500"}`}></span>
        ...
        <div className="px-5 text-xs">
          <p className="mb-2 font-medium">Media</p>
          {mediaImages.length > 0 ? (
            ...
            <p className="mt-2 text-gray-400 text-xs italic">No media shared yet</p>
          )}
        </div>
        <button
          onClick={logout}
          className="absolute bottom-5 left-1/2 transform -translate-x-1/2 bg-gradient-to-r from-purple-400 to-violet-600
        text-white border-none text-sm font-light py-2 px-20 rounded-full cursor-pointer whitespace-nowrap"
        >
          Logout
        </button>
      </div>
